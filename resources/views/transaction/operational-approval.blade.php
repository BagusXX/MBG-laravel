...
@section('content')
                                    ...
                                    <td class="text-center">
                                        ...
                                        {{-- <div class="btn-group-vertical btn-group-sm">
                                            ...
                                            <div class="text-right">
                                                <a href="{{ route('transaction.operational-approval.invoice-parent', $item->id) }}"
    ...
    <x-modal-form id="modalAddOperational" size="modal-lg" title="Tambah Pengajuan Operasional" action="#"
        submitText="Simpan" method="POST">
        ...
        <div class="d-flex align-items-center">
            ...
            <div class="form-group">
                <label>Kode</label>
                <input type="text" class="form-control" value="OPR001" readonly required style="background:#e9ecef">
            </div>

            {{-- TANGGAL --}}
            <div class="form-group flex-fill ml-2">
                <label>Tanggal</label>
                <input type="date" class="form-control" value="{{ now()->toDateString() }}" required>
            </div>
        ...
        <div class="form-group">
            ...
            <input type="number" min="0" class="form-control" placeholder="Masukkan total biaya" required>
        </div>
    ...
    @foreach($submissions as $item)
        <x-modal-detail id="modalDetail{{ $item->id }}" size="modal-lg" title="Detail Pengajuan: {{ $item->kode }}">
            ...
            @if(
                ...
                <div class="text-right mb-3">
                    <button class="btn btn-danger btn-sm btnApproval" data-id="{{ $item->id }}" data-status="ditolak">
            ...
            @if ($item->status === 'diproses' && $allChildDone)
                <div class="text-right mb-3">
                    <button class="btn btn-success btn-md btnApproval" data-id="{{ $item->id }}" data-status="selesai">
            ...
            @endif
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped mt-2">
                                ...
                                <tr>
                                    @if(in_array($item->status, ['diajukan', 'diproses']))
                                        <td class="text-center align-middle">
                                            <input type="checkbox" name="items[]" value="{{ $detail->id }}"
                                                class="item-check-{{ $item->id }}" {{ $isProcessed ? 'disabled checked' : '' }}>
                                        </td>
                                    @endif
                                    <td>
                                    ...
                                    <td width="180">
                                        <div class="input-group input-group-sm">
                                            ...
                                            <input type="hidden" name="details[{{ $detail->id }}][qty]" value="{{ $detail->qty }}">
                                            <input type="number" name="details[{{ $detail->id }}][harga_dapur]"
                                                class="form-control form-control-sm text-right harga-dapur"
                                                value="{{ $detail->harga_dapur }}" {{ $item->status === 'selesai' ? 'readonly' : '' }}
                                                min="0" step="0.01" data-qty="{{ $detail->qty }}" data-id="{{ $detail->id }}">
                                        </div>
                                        {{--
                                    <td width="160">
                                        <input type="number" name="details[{{ $detail->id }}][harga_mitra]"
                                            class="form-control form-control-sm text-right" value="{{ $detail->harga_mitra }}" {{
                                    ...
                                    <td class="text-right">Rp {{ number_format($detail->qty * $detail->harga_satuan, 0, ',', '.') }}
                                    </td>
                                </tr>
                            @empty
                                <tr>
                        ...
                        {{-- <tfoot>
                            <tr>
                                <th colspan="{{ in_array($item->status, ['diajukan', 'diproses']) ? '4' : '3' }}"
                                    class="text-right">Total Keseluruhan</th>
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                ...
                                <div>
                                    ...
                                    @if(
                                        ...
                                        <button class="btn btn-xs btn-outline-danger ml-2 btnDeleteChild" data-id="{{ $child->id }}"
    ...
    <x-modal-form id="modalApprovalOperational" title="Konfirmasi Approval" action="#" submitText="Ya, Lanjutkan"
        method="POST">
        @csrf
        @method('PATCH')
        ...
        <div class="form-group d-none" id="keterangan_wrapper">
            ...
            <textarea name="keterangan" class="form-control" placeholder="Masukkan alasan penolakan"></textarea>
        </div>
    ...
    <x-modal-detail id="modalSupplierRequired" title="Supplier Belum Dipilih" size="modal-md">
    ...
    <x-modal-form id="modalDeleteChild" title="Hapus Approval Supplier" action="" submitText="Ya, Hapus" method="POST">
        @csrf
        @method('DELETE')
